...
const CheckboxInput: FC<CheckboxInputProps> = ({ isChecked, onChange, radio, value }) => (
  <input
  ...
  if (circle) {
    return (
      <animated.svg
        ...
        <animated.circle cx="10" cy="10" r="6.55" fill={isChecked ? '#0D6EFD' : '#fff'} />
      </animated.svg>
    )
  }
  ...
  return (
    <label className={'inline-flex items-center select-none cursor-pointer'}>
      <CheckboxInput
        ...
        onChange={onChange}
      />
      <CheckboxFigure circle={circle} isChecked={radio ? isCheckRadio : !!isChecked} />
      {children}
    </label>
  )
}
